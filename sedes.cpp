...
void Sedes::consultaNodo(const QString &nombre) {
    ...
    if (consultar_nodo.exec() and consultar_nodo.first()) {
        ...
        consultar_telefono.bindValue(":idNodo", QString(id_nodo));
        ...
        if (consultar_poblacion.exec()) {
            if (consultar_poblacion.first()) {
                ...
                if (consultar_municipio.exec()) {
                    if (consultar_municipio.first()) {
                        ...
                        ui -> lineEdit_municipio_web -> setText(consultar_nodo.value(NUM_COL_MUNICIPIO_WEB).toString());
            ...
            consultar_email.bindValue(":idNodo", QString(id_nodo));
...
void Sedes::activa_botones() {
    ...
    ui -> pB_tablon -> setEnabled(sede_activa);
    ui -> pB_sede -> setEnabled(sede_activa);
    ui -> pB_portalTransparencia -> setEnabled(sede_activa);
}
